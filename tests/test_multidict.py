...
def _test_unicode_dict(decode_param_names=False):
    ...
    assert list(d.items()) == [(k('a'), 'a test')]
    list(map(assert_key_str, list(d.keys())))
    list(map(assert_unicode, list(d.values())))

    d[b'b'] = b'2 test'
    d[b'c'] = b'3 test'
    assert list(d.items()) == [(k('a'), 'a test'), (k('b'), '2 test'), (k('c'), '3 test')]
    list(map(assert_unicode_item, list(d.items())))

    d[k('b')] = b'4 test'
    assert list(d.items()) == [(k('a'), 'a test'), (k('c'), '3 test'), (k('b'), '4 test')], list(d.items())
    list(map(assert_unicode_item, list(d.items())))
    ...
    assert list(d.items()) == [(k('a'), 'a test'), (k('c'), '3 test'), (k('b'), '4 test'),
                         (k('b'), '5 test')]
    list(map(assert_unicode_item, list(d.items())))

    del d[k('b')]
    assert list(d.items()) == [(k('a'), 'a test'), (k('c'), '3 test')]
    list(map(assert_unicode_item, list(d.items())))
    assert d.pop('xxx', '5 test') == '5 test'
    assert isinstance(d.pop('xxx', '5 test'), six.text_type)
    ...
    d[(1, None)] = (None, 1)
    assert list(d.items()) == [(k('a'), 'a test'), (k('y'), 'y test'), (k('x'), 'x test'),
                         ((1, None), (None, 1))]
    item = list(d.items())[-1]
